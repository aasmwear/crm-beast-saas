...
/**
 * Archive comments older than the configured retention window into comments_archive.
 * Dry-run by default; use --execute to move rows in batches.
 */
final class LifecycleArchiveCommentsCommand extends Command
{
    protected $signature = 'lifecycle:archive-comments
    ...
    protected $description = 'Archive aged comments to comments_archive (dry-run by default; use --execute)';

    public function handle(WarmTableArchiver $archiver): int
    {
        $policy = $this->loadPolicy('comments');
        if ($policy === null) {
            return self::FAILURE;
        }

        if (! $policy->isWarm()) {
            $this->error('comments must be category "warm" in config/lifecycle.php for archival.');

            return self::FAILURE;
        }

        if (! Schema::hasTable('comments')) {
            $this->warn('Table comments does not exist.');

            return self::SUCCESS;
        }

        if (! Schema::hasTable('comments_archive')) {
            $this->warn('Table comments_archive does not exist. Run migrations.');
        ...
        $this->info('Lifecycle Archive: comments → comments_archive');
