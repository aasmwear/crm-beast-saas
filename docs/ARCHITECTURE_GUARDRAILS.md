...
## Partition readiness (future LIST / RANGE by `organization_id`)

- **Strategy (when / how, no DDL executed):** See [`docs/PARTITIONING_STRATEGY.md`](PARTITIONING_STRATEGY.md) for partition candidates, key choice per table, threshold triggers, operations, risks, and rollout phases.
- **Not implemented:** Tables are not physically partitioned yet. This section documents candidates and access rules so a future migration can attach partitions without rewriting application code.
- **Candidate hot tables (row growth):** `tasks` (soft-deleted rows retained until purge), `activities` (warm → `activities_archive`), `stripe_webhook_events` (cold prune), `audit_logs`, `comments`, `notifications`, `activities_archive` (read/cold).
- **Access rule:** Any query that can scan more than one tenant’s rows MUST filter by **`organization_id`** first (or use a key that implies it, e.g. `project_id` only after validating the project belongs to the route org). Platform-wide jobs that intentionally scan all tenants (webhook summary rebuild, lifecycle prune) are exceptions and must remain operator-only / scheduled.
- **Indexes (PostgreSQL):** Migration `2026_04_11_000001_partition_readiness_indexes` adds tenant-aligned composites: `stripe_webhook_events` — partial `(organization_id, created_at) WHERE status = 'failed'`, `(organization_id, type)`, `(organization_id, processed_at DESC NULLS LAST)`; replaces standalone `(organization_id)` and non-tenant `(subject_type, subject_id)` on `activities` with `(organization_id, subject_type, subject_id)`; `tasks` — `(organization_id, created_at) WHERE deleted_at IS NULL` alongside existing `(organization_id, status)` and GIN on `assignees`.
- **Morph relations:** `Project::activities()` and `Task::activities()` constrain `activities.organization_id` to the parent’s org; project show eager-load applies the same filter (defense in depth).
- **Future partition shape:** Prefer **RANGE** or **LIST** on `organization_id` (or hash of `organization_id` into a small modulus) so partition pruning matches tenant-scoped queries. Sub-partitioning by month on `created_at` is optional for very large single-tenant shards.
...
- **Platform Org Health partial hybrid:** `OrgHealthController` uses `org_daily_metrics.users_count` for `seats_active` when a matching row exists, then falls back to the live `organization_user` + `users.client_id` count. **Tier / config:** {@see \App\Support\OrgSnapshotReadMode::shouldUseSnapshot} — **large** and **enterprise** orgs (or `ORG_SNAPSHOT_READ_ALL_TENANTS=true` / `config/org_daily_metrics.php` `snapshot_only_all_tenants`) use the **latest** snapshot row per org (max `metric_date`) so a missed nightly job does not force a live seat aggregation; **small** and **medium** orgs keep **yesterday-only** snapshot alignment with the scheduled `metrics:snapshot-orgs` default date. Billing, Stripe presence, subscription status, seat limits, storage pressure, and webhook aggregates remain live. No other snapshot columns are forced into this dashboard until semantics clearly match.
- **Snapshot-only read mode (v1):** `OrgSnapshotReadMode::shouldUseSnapshot()` gates snapshot-first **latest-row** reads for large/enterprise (optional global override). Accuracy is as-of the newest `metric_date` row (typically EOD); it can lag live membership by up to one snapshot interval when the job is healthy, or longer if snapshots stop — live fallback applies when no row exists. **System Performance** totals (`total_users`, storage) have no snapshot column mapping; they stay live.
- **Migration path:** Remaining platform dashboards (e.g. system performance platform totals, revenue) stay live unless a column maps cleanly to operator expectations.
